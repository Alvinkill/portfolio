...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const observer = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                entry.target.style.boxShadow = '0 0 30px rgba(0, 212, 255, 0.3)';
                setTimeout(() => {
    ...
    // Mouse Follow Effect for Dynamic Background
    ...
    // Add CSS custom properties for mouse position
    ...
    mouseStyle.textContent = `
        ...
        body::before {
            background: 
                radial-gradient(circle at calc(var(--mouse-x) * 100%) calc(var(--mouse-y) * 100%), rgba(0, 212, 255, 0.15) 0%, transparent 30%),
                radial-gradient(circle at calc((1 - var(--mouse-x)) * 100%) calc(var(--mouse-y) * 100%), rgba(255, 107, 107, 0.15) 0%, transparent 30%),
                radial-gradient(circle at calc(var(--mouse-x) * 100%) calc((1 - var(--mouse-y)) * 100%), rgba(78, 205, 196, 0.15) 0%, transparent 30%);
        }
    `;
